...
namespace BoardStats.Data.Services
{
    public class PlayersService : IPlayersService
    {
        ...
        public async Task AddNewPlayerAsync(string userName)
        {
            ...
            await _db.Players.AddAsync(newPlayer2);
            await _db.Players.AddAsync(newPlayer3);
        ...
        public async Task<List<Player>> GetAllPlayersByUserId(string userId, string userRole)
        {
            ...
            var players = await _db.Players.Where(n => n.UserId == userId && n.PlayerTag!="Game_Players").ToListAsync();
            if (userRole == "Admin")
            {
                players = await _db.Players.Where(n=>n.PlayerTag!="Game_Players").ToListAsync();
            }
