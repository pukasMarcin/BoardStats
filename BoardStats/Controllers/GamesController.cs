...
namespace BoardStats.Controllers
{
    ...
    public class GamesController : Controller
    {
        ...
        {
            IEnumerable<Boardgames> allGames;
            if (Id == 1)
            {
               allGames = await _serviceBg.GetAllNoAdds();
                ViewData["Adds"] = "NoAdds";
            }
            else
            {
                allGames = await _serviceBg.GetAll();
                ViewData["Adds"] = "Adds";
            }
   ...
   new { controller = "Games", action = "AddGameBgg", id= "2" }));
        ...
        public async Task <IActionResult> SeedBgg(string Bgg_Id)
        {
            ...
            IEnumerable<Boardgames> gamesNoExpansion; 
            ...
            var AllGames = await _serviceBg.GetAllToSelector();
      ...
      public async Task<IActionResult> NoBgg()
        {
            ...
            keeper.Name = "Wprowadź nazwę gry";
            keeper.statsVM = statsDroop;
            keeper.winVM = winDroop;
            return View("AddNewGame", keeper);
        ...
        public async Task<IActionResult> Create(BoardViewModel board)
        {
            ...
            return RedirectToAction("Index", "Games", new {Id=1});
